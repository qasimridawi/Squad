...
# --- CONFIG ---
SECRET_KEY = "squad-platinum-v12"
ALGORITHM = "HS256"
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token")

# --- DATABASE ---
database_url = "sqlite:///./squad_v12.db"
...
class User(Base):
    ...
    bio = Column(String, default="Squad Member") # New: User Bio
    is_admin = Column(Boolean, default=False)

class Hangout(Base):
    __tablename__ = "hangouts_v12"
    id = Column(Integer, primary_key=True, index=True)
    title = Column(String)
    location = Column(String)
    host_username = Column(String)
    image_data = Column(Text, nullable=True)
    video_data = Column(Text, nullable=True) # New: Video Support
...
class Participant(Base):
    __tablename__ = "participants_v12"
    id = Column(Integer, primary_key=True, index=True)
    hangout_id = Column(Integer, ForeignKey("hangouts_v12.id"))
    username = Column(String)
    user_avatar = Column(Text, nullable=True)
    hangout = relationship("Hangout", back_populates="participants")

class Message(Base):
    __tablename__ = "messages_v12"
    id = Column(Integer, primary_key=True, index=True)
    hangout_id = Column(Integer, ForeignKey("hangouts_v12.id"))
    username = Column(String)
    user_avatar = Column(Text, nullable=True)
...
class DirectMessage(Base):
    __tablename__ = "direct_messages_v12"
    id = Column(Integer, primary_key=True, index=True)
...
# --- ENDPOINTS ---
@app.get("/health")
def health(): return {"status": "ok", "version": "v12"}
